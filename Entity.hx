...
class Entity extends Tweener
{
	...
	/**
	 * If grid is detected around the 
	 */
	public var gridDetected:Bool;
	...
	/**
	 * Function to check for Grid tiles around the entity standing tile. (Grid has to be set as mask and given an instance name!)
	 * @param	type		The collsion type you set for the grid.
	 * @param	tileSize	Size of a tile.
	 * @param	name		Instance name of the grid entity.
	 * @return	If grid got detected.
	 */
	public function gridDetectiv(type:String, tileSize:Int, name:String):Bool {
		
		if (HXP.scene.getInstance(name) != null) {
			
			//get the values
			var bernd:Entity = HXP.scene.getInstance(name);
			var gritty:Grid = cast(bernd.mask, Grid);
			
			var sc:Int = Math.round(this.x / tileSize - 0.5);
			var sr:Int = Math.round(this.y / tileSize - 0.5);	
			
			//check for grid
			if (gritty.getTile(sc - 1, sr - 1) || gritty.getTile(sc, sr - 1) || gritty.getTile(sc + 1, sr - 1) ||
				gritty.getTile(sc - 1, sr) || gritty.getTile(sc + 1, sr) || gritty.getTile(sc - 1, sr + 1) || 
				gritty.getTile(sc, sr + 1) || gritty.getTile(sc + 1, sr + 1) || gritty.getTile(sc - 2, sr - 2) || 
				gritty.getTile(sc - 1, sr - 2) || gritty.getTile(sc, sr - 2) || gritty.getTile(sc + 1, sr - 2) ||
				gritty.getTile(sc + 2, sr - 2) || gritty.getTile(sc - 2, sr - 1) || gritty.getTile(sc - 2, sr) ||
				gritty.getTile(sc - 2, sr + 1) || gritty.getTile(sc + 2, sr - 1) || gritty.getTile(sc + 2, sr) ||
				gritty.getTile(sc + 2, sr + 1) || gritty.getTile(sc - 2, sr + 2) || gritty.getTile(sc - 1, sr + 2) || 
				gritty.getTile(sc, sr + 2) || gritty.getTile(sc + 1, sr + 2) || gritty.getTile(sc + 2, sr + 2))			{
	...
	/**
	 * Check of stucking entities and move them.
	 * @param 	typ		The type of entity to check.
	 * @param	wen		1 - move the entity colliding with, 2 - move this entity.
	 * @param	knock	The multiplicator for moving the Entity.
	 * @param	block	Which types to not pass while moving (moveBy).
	 */
	public function stuckSolve(type:String, wen:Int = 1, knock:Int = 1, block:String = "") {
		...
		if (type == "solid") { 
			...
			//get the values
			var bernd:Entity = collide(type, this.x, this.y);	//muss möglciherweise wie bei gridDetectiv (eine function drüber) auf HXP.scene.getInstance("welt") geändert werden.
			var gritty:Grid = cast(bernd.mask, Grid);
			
			var sc:Int = Math.round(this.x / 16 - 0.5);
			var sr:Int = Math.round(this.y / 16 - 0.5);
			
			//apply some solution
			//erster kreis
			if (gritty.getTile(sc - 1, sr - 1)) { moveBy( knock, knock, block); }
			if (gritty.getTile(sc, sr - 1)) 	{ moveBy( 0, knock, block); }
			if (gritty.getTile(sc + 1, sr - 1)) { moveBy( -knock, knock, block); }
			if (gritty.getTile(sc - 1, sr)) 	{ moveBy( knock, 0, block); }
			if (gritty.getTile(sc + 1, sr)) 	{ moveBy( -knock, 0, block); }
			if (gritty.getTile(sc - 1, sr + 1)) { moveBy( knock, -knock, block); }
			if (gritty.getTile(sc, sr + 1)) 	{ moveBy( -knock, 0, block); }
			if (gritty.getTile(sc + 1, sr + 1))	{ moveBy( -knock, -knock, block); }
			
			//zweiter kreis
			if (gritty.getTile(sc - 2, sr - 2)) { moveBy( knock, knock, block); }
			if (gritty.getTile(sc - 1, sr - 2)) { moveBy( knock, knock, block); }
			if (gritty.getTile(sc, sr - 2)) 	{ moveBy( 0, knock, block); }
			if (gritty.getTile(sc + 1, sr - 2)) { moveBy( -knock, knock, block); }
			if (gritty.getTile(sc + 2, sr - 2)) { moveBy( -knock, knock, block); }
			if (gritty.getTile(sc - 2, sr - 1)) { moveBy( knock, knock, block); }
			if (gritty.getTile(sc - 2, sr)) 	{ moveBy( knock, 0, block); }
			if (gritty.getTile(sc - 2, sr + 1))	{ moveBy( knock, -knock, block); }		
			if (gritty.getTile(sc + 2, sr - 1)) { moveBy( -knock, knock, block); }
			if (gritty.getTile(sc + 2, sr)) 	{ moveBy( -knock, 0, block); }
			if (gritty.getTile(sc + 2, sr + 1)) { moveBy( -knock, -knock, block); }
		...
		if (collide(type, this.x, this.y) != null && type != "solid") {
			
			var bernd = collide(type, this.x, this.y);
			var winkel = HXP.angle(this.x, this.y, bernd.x, bernd.y);
			winkel *= Math.PI / 180;
			
			if (wen == 1) {
				
				bernd.moveBy( Math.sin(winkel) * knock , -Math.cos(winkel) * knock, block, true);
			}
			else if (wen == 2) {
				
				this.moveBy( -Math.cos(winkel) * knock, Math.sin(winkel) * knock, block, true);
			}
			else throw "Moving entity not clarified. Use 1 or 2.";	
		}
	}
